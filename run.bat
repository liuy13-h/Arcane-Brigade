...
rem Requires: JDK 21 (default D:\develop) and `mvn install` already run once.
setlocal
...
if errorlevel 1 (
  echo.
  echo [ERROR] Game exited with an error. If it says "class not found",
  echo         run `mvn install -DskipTests` in this folder first.
  pause
)
endlocal
